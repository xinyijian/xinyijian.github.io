...
- (void)viewDidLoad {
    ...
    [PattAmapLocationManager singleton].locationBlock = ^(CLLocation *location,NSString * address) {
        ...
        seachBar.cityName = [PattAmapLocationManager singleton].city;
        if ([PattAmapLocationManager singleton].city.length > 3) {
            seachBar.cityName = [NSString stringWithFormat:@"%@..",[[PattAmapLocationManager singleton].city substringWithRange:NSMakeRange(0, 3)]];
        }
    };
    WS(weakSelf);
