...
@implementation ListAddressTableViewCell
...
- (void)initUI{
    ...
    [_address activateConstraints:^{
        ...
        [_address.right_attr equalTo:self.contentView.right_attr constant:-45];
    }];
    ...
    [line activateConstraints:^{
        ...
        [line.right_attr equalTo:self.contentView.right_attr constant:-45];
    ...
    [_EidtBtn activateConstraints:^{
        [_EidtBtn.right_attr equalTo:self.contentView.right_attr constant:-15];
        [_EidtBtn.centerY_attr equalTo:self.EidtBtn.centerY_attr];
        _EidtBtn.width_attr.constant = 16;
        [_EidtBtn.height_attr equalTo:self.contentView.height_attr];
    }];
    [_EidtBtn setImage:[UIImage imageNamed:@"icon-Edit"] forState:UIControlStateNormal];
    [_EidtBtn addTarget:self action:@selector(eidtAddress:) forControlEvents:UIControlEventTouchUpInside];
}
- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
}
- (void)eidtAddress:(UIButton *)btn
{
    BLOCK_EXEC(_cellBlock);
}
