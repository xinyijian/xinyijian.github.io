...
- (id)initWithFrame:(CGRect)frame
{
    ...
    self.datePickerStyle =DateStyleShowYearMonthDayHourMinute;
    [self setupUI];
    [self defaultConfig];
    _startDate = [self.scrollToDate dateWithFormatter:_dateFormatter];
    BLOCK_EXEC(_pickeblock,_startDate);
...
//    self.frame=CGRectMake(0, 0, kScreenWidth, 52.5);
    
    
//    self.backgroundColor = RGBA(0, 0, 0, 0);
    [self layoutIfNeeded];
    [self addSubview:self.datePicker];
    [_datePicker activateConstraints:^{
        _datePicker.top_attr = self.top_attr;
        _datePicker.right_attr = self.right_attr;
        _datePicker.left_attr = self.left_attr;
        _datePicker.height_attr = self.height_attr;
    }];
}
...
-(void)defaultConfig {
    ...
    for (NSInteger i=MINYEAR; i<=MAXYEAR; i++) {
        ...
        long long times = [[NSDate date] timeIntervalSince1970] * 1000;
        ...
        NSDate *datesa = [dateFormatter dateFromString:@"2018-03-21 14:42"];
        long long tioo = [datesa timeIntervalSince1970] * 1000;
        NSLog(@"%lld",tioo);
        NSLog(@"%lld",times);
    long long sss = tioo - times;
    NSLog(@"%lld",sss);
    ...
    if (!self.minLimitDate) {
        self.minLimitDate = [NSDate date:st WithFormat:@"yyyy-MM-dd HH:mm"];
//        self.minLimitDate = datesa;
    }
}
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    switch (self.datePickerStyle) {
        case DateStyleShowYearMonthDayHourMinute:
            ...
            return 3;
        case DateStyleShowMonthDayHourMinute:
            [self addLabelWithName:@[NSLocalizedString(@"月",nil),NSLocalizedString(@"日",nil),NSLocalizedString(@"时",nil),NSLocalizedString(@"分",nil)]];
            return 4;
        case DateStyleShowYearMonthDay:
            [self addLabelWithName:@[NSLocalizedString(@"年",nil),NSLocalizedString(@"月",nil),NSLocalizedString(@"日",nil)]];
            return 3;
        case DateStyleShowYearMonth:
            [self addLabelWithName:@[NSLocalizedString(@"年",nil),NSLocalizedString(@"月",nil)]];
            return 2;
        case DateStyleShowMonthDay:
            [self addLabelWithName:@[NSLocalizedString(@"月",nil),NSLocalizedString(@"日",nil)]];
            return 2;
        case DateStyleShowHourMinute:
            [self addLabelWithName:@[NSLocalizedString(@"时",nil),NSLocalizedString(@"分",nil)]];
            return 2;
        default:
            return 0;
    }
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    [customLabel setFont:fontStely(@"PingFangSC-Regular", 11)];
    NSString *title;
    NSString * daysa = NSLocalizedString(@"日",nil);
    NSString * mintes = NSLocalizedString(@"时",nil);
    NSString * fen = NSLocalizedString(@"分",nil);
    switch (self.datePickerStyle) {
        case DateStyleShowYearMonthDayHourMinute:
//            if (component==0) {
...
//            }
            if (component==0) {
                title = [NSString stringWithFormat:@"%@%@",_dayArray[row],daysa];
            }
            if (component==1) {
                title = [NSString stringWithFormat:@"%@%@",_hourArray[row],mintes];
            }
            if (component==2) {
                title = [NSString stringWithFormat:@"%@%@",_minuteArray[row],fen];
            }
            break;
        case DateStyleShowYearMonthDay:
            if (component==0) {
...
//        _datePicker.height =  52.5;
        _datePicker.showsSelectionIndicator = YES;
        _datePicker.delegate = self;
        _datePicker.dataSource = self;
//        _datePicker.centerXBee =  SCREEN_Width / 2.0f;
//        _datePicker.widthBee = 187.5;
