...
- (void)viewDidLoad {
    [super viewDidLoad];
//    _actionAddress = 0;
    _seachBarCity = [PattAmapLocationManager singleton].presentLocation.city;
...
- (void)pattaMapLocationInit{
    ...
    [PattAmapLocationManager singleton].locationBlock = ^(CLLocation *location,NSString * address) {
        _seachBar.cityName = [PattAmapLocationManager singleton].presentLocation.city;

        if ([PattAmapLocationManager singleton].presentLocation.city.length > 3) {
            _seachBar.cityName = [NSString stringWithFormat:@"%@..",[[PattAmapLocationManager singleton].presentLocation.city substringWithRange:NSMakeRange(0, 3)]];
        }
        ...
        [weakSelf NotSupport: [PattAmapLocationManager singleton].presentLocation.adcode];
