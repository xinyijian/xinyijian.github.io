...
//居中卡片宽度与据屏幕宽度比例
static float CardWidthScale = (float)280/375;
...
//卡片间隔
- (float)cellMargin {
    return 1;//(self.collectionView.bounds.size.width - [self cellWidth])/7
}
...
//cell大小
- (CGSize)itemSize {
    return CGSizeMake([self cellWidth],self.collectionView.bounds.size.height);
}
