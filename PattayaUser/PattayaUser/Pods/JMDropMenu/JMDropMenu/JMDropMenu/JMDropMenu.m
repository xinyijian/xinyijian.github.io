...
- (instancetype)initWithFrame:(CGRect)frame ArrowOffset:(CGFloat)arrowOffset TitleArr:(NSArray *)titleArr ImageArr:(NSArray *)imageArr Type:(JMDropMenuType)type LayoutType:(JMDropMenuLayoutType)layoutType RowHeight:(CGFloat)rowHeight Delegate:(id<JMDropMenuDelegate>)delegate {
    if (self = [super initWithFrame:frame]) {
        //初始化赋值
        self.frame = frame;
        CGPoint cents = self.center;
        self.center = CGPointMake([[UIScreen mainScreen] bounds].size.width / 2.0f, cents.y);
        ...
        for (int i = 0; i < titleArr.count; i++) {
            ...
            if (i == 0) {
                ...
                [tempDict setObject:@(1) forKey:@"isSeletced"];
            } else
            {
                [tempDict setObject:@(0) forKey:@"isSeletced"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (JMDropMenuModel * models in self.titleImageArrM) {
        models.isSeletced = 0;
    }
    model.isSeletced = 1;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.contentView addSubview:_backgrouView];
        _backgrouView.backgroundColor =  [UIColor whiteColor];
...
- (void)layoutSubviews {
    ...
    CGPoint centsela = self.contentView.center;
    self.screenCenter = self.contentView.frame.size.height * 0.5;
    _backgrouView.frame = CGRectMake(7.5, self.screenCenter - 10, self.contentView.frame.size.width - 15, self.contentView.frame.size.height);
    ...
    CGPoint titleCent = self.titleL.center;
    titleCent.y = centsela.y;
    self.titleL.center = _backgrouView.center;
...
//        self.titleL.frame = CGRectMake(10, self.screenCenter - 10, self.frame.size.width, 20);
    ...
    if (model.isSeletced == 1) {
        _backgrouView.backgroundColor =  UIColorFromRGB(0xf8f8f8);
        self.titleL.textColor = UIColorFromRGB(0x43496a);

    } else
    {
        _backgrouView.backgroundColor =  [UIColor whiteColor];
        self.titleL.textColor = UIColorFromRGB(0x8a8fab);
