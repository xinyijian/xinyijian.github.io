...
- (void)getPushMessageRequest:(NSDictionary *)dic Success:(void (^)(NSURLSessionDataTask *operation,NSDictionary *ret))successResult failure:(void (^)(NSURLSessionDataTask *operation,NSError *error))failureResult
{
    [[PattayaHttpRequest sharedHttpManager] getWithUrlPath:[NSString stringWithFormat:@"/user/getPushMessage"] parameter:dic isNeedIndicator:YES success:^(NSURLSessionDataTask *operation, NSDictionary *responesObject) {
        successResult(operation,responesObject);
    } failure:^(NSURLSessionDataTask *operation, NSError *error) {
        failureResult(operation,error);
    }];
