...
@interface DD_PageControls : UIPageControl

@end
@implementation DD_PageControls
- (instancetype)init
{
    self = [super init];
    if (self) {
...
-(void)setCurrentPage:(NSInteger)currentPage{
    ...
    for (NSUInteger subviewIndex = 0; subviewIndex < [self.subviews count]; subviewIndex++) {
        ...
        if (subviewIndex == currentPage)
        {
            ...
            subview.layer.borderColor = UIColorFromRGB(0x43496A).CGColor;
...
@property (nonatomic, strong) DD_PageControls* pageControl;
...
//    [self addSubview:pageControl];
    ...
    _pageControl = [[DD_PageControls alloc] init];
    _pageControl.frame = CGRectMake(210, 0, 375, 20);//指定位置大小
    _pageControl.numberOfPages = 3;//指定页面个数
    _pageControl.currentPage = 0;//指定pagecontroll的值，默认选中的小白点（第一个）
    //添加委托方法，当点击小白点就执行此方法
    
    _pageControl.pageIndicatorTintColor = UIColorFromRGB(0xD8D8D8);// 设置非选中页的圆点颜色
...
//        y = CGRectGetHeight(self.frame) - size.height - self.pageControlAlignmentOffset.height;
        y = CGRectGetHeight(self.frame) - 22.5f;
